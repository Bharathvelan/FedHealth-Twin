...
if 'all_recs'      not in st.session_state: st.session_state.all_recs      = None
if 'risk_diseases' not in st.session_state: st.session_state.risk_diseases = []
...
with st.sidebar:
    ...
    if st.session_state.predicted:
        ...
        <div style="text-align:center;margin-top:10px;padding:8px;
             background:rgba(0,201,87,0.1);border-radius:8px;
             border:1px solid rgba(0,201,87,0.3);color:#00e676;font-size:0.78rem;">
            ✅ Prediction ready!<br>Use tabs to explore results.
...
disease_recs = {
    'diabetes':     ["Monitor blood sugar daily","Reduce refined carbs and sweets",
                     "Exercise 30 min daily","Consult endocrinologist",
                     "Check HbA1c every 3 months"],
    'cvd':          ["Reduce saturated fat intake","Stop smoking immediately",
                     "30 min cardio daily","Monitor cholesterol monthly",
                     "Consult cardiologist urgently"],
    'ckd':          ["Reduce protein and salt intake","Monitor creatinine weekly",
                     "Control blood pressure strictly","Avoid nephrotoxic drugs",
                     "Consult nephrologist immediately"],
    'hypertension': ["Reduce salt intake drastically","Exercise daily",
                     "Monitor BP twice daily","Avoid stress and alcohol",
                     "Consult physician for medication"],
    'fatty_liver':  ["Avoid alcohol completely","Lose weight gradually",
                     "Reduce fatty and fried foods","Exercise 5 days per week",
...
if predict_btn:
    ...
    if not all_recs:
        all_recs = ["Maintain regular exercise","Eat balanced diet",
                    "Get health checkup every 6 months","Stay hydrated"]
    ...
    st.session_state.patient_info  = {
        ...
        'alt_enzyme': alt_enzyme, 'smoker': smoker,
        'diabetes_history': diabetes_history, 'family_history': family_history,
        'alcohol': alcohol, 'physical_activity': physical_activity,
        'pa': pa, 'pdf_scaled': pdf_scaled.tolist()
    }
...
if st.session_state.predicted:
    ...
    all_recs      = st.session_state.all_recs
    risk_diseases = st.session_state.risk_diseases
    safe_diseases = [d for d,p in predictions.items() if p==0]
    p             = st.session_state.patient_info
    patient_name  = p.get('patient_name','Patient')
    age           = p.get('age', 45)
    gender        = p.get('gender','Male')
    bmi           = p.get('bmi', 25.0)
    blood_pressure= p.get('blood_pressure', 120)
    blood_sugar   = p.get('blood_sugar', 100)
    cholesterol   = p.get('cholesterol', 180)
    heart_rate    = p.get('heart_rate', 75)
    spo2          = p.get('spo2', 98.0)
    creatinine    = p.get('creatinine', 1.0)
    hba1c         = p.get('hba1c', 5.5)
    triglycerides = p.get('triglycerides', 150)
    alt_enzyme    = p.get('alt_enzyme', 30)
    smoker        = p.get('smoker', False)
    diabetes_history = p.get('diabetes_history', False)
    family_history   = p.get('family_history', False)
    alcohol          = p.get('alcohol', False)
    physical_activity= p.get('physical_activity','Sedentary (0)')
    pdf_scaled       = np.array(p.get('pdf_scaled', [[0]*17]))
    ...
    tab1,tab2,tab3,tab4,tab5,tab6,tab7 = st.tabs([
        "📊 Analysis", "📋 Vitals", "💡 Recommendations",
        "🔍 SHAP & XAI", "🎙️ Voice Assistant",
        "🤖 AI Report", "📄 Download"
    ])
    ...
    with tab1:
        ...
        with c1:
            ...
            pp = probs_list+[probs_list[0]]; aa = angles+[angles[0]]
            ...
            ax.set_xticks(angles); ax.set_xticklabels(disease_names_list,color='white',fontsize=9)
            ax.set_ylim(0,100); ax.set_yticks([20,40,60,80,100])
            ax.set_yticklabels(['20','40','60','80','100'],color='#888',fontsize=7)
            ax.grid(color='#333',linewidth=0.8); ax.spines['polar'].set_color('#333')
            ax.set_title(f"Risk Radar — {patient_name}",color='white',fontweight='bold',fontsize=11,pad=15)
            fig.tight_layout(); st.pyplot(fig,use_container_width=True); plt.close()
        with c2:
            colors_bar=['#e53935' if predictions[d]==1 else '#2e7d32' for d in disease_info.keys()]
            fig,ax = plt.subplots(figsize=(5,5),facecolor='#16213e'); ax.set_facecolor('#16213e')
            bars=ax.barh(disease_names_list,probs_list,color=colors_bar,edgecolor='#ffffff11',height=0.5)
            for bar,val in zip(bars,probs_list):
                ax.text(min(val+1,95),bar.get_y()+bar.get_height()/2,
                        f'{val:.1f}%',va='center',color='white',fontweight='bold',fontsize=9)
            ax.axvline(x=50,color='#ffcc02',linewidth=1.5,linestyle='--',alpha=0.7,label='Risk Threshold')
            ax.set_xlim(0,110); ax.set_xlabel("Risk Probability (%)",color='#aaa',fontsize=9)
            ax.set_title("Disease Risk Levels",color='white',fontweight='bold',fontsize=11,pad=10)
        ...
        with c2:
            ...
            if all(s>0 for s in sizes):
                ...
                ax.text(0.5,0.5,'✅ All 5\nSAFE',ha='center',va='center',color='#00e676',
                        fontsize=14,fontweight='bold',transform=ax.transAxes)
            else:
                ax.text(0.5,0.5,'🚨 All 5\nAT RISK',ha='center',va='center',color='#ff5252',
                        fontsize=14,fontweight='bold',transform=ax.transAxes)
            ax.set_title("Disease Risk Summary",color='white',fontweight='bold',fontsize=11)
            fig.tight_layout(); st.pyplot(fig,use_container_width=True); plt.close()
    ...
    with tab2:
        ...
        st.markdown('<div class="section-header">📊 Vitals vs Normal Range</div>',
                    unsafe_allow_html=True)
        vitals_data={'BMI':(bmi,25),'BP':(blood_pressure,120),'Sugar':(blood_sugar,140),
                     'Cholesterol':(cholesterol,200),'HR':(heart_rate,100),
                     'HbA1c(x10)':(hba1c*10,57),'Creatinine(x10)':(creatinine*10,12)}
        fig,ax=plt.subplots(figsize=(10,4),facecolor='#16213e'); ax.set_facecolor('#16213e')
        vnames=list(vitals_data.keys()); vvals=[vitals_data[k][0] for k in vnames]
        vnorms=[vitals_data[k][1] for k in vnames]; x=np.arange(len(vnames)); w=0.35
        vcolors=['#e53935' if v>n else '#2e7d32' for v,n in zip(vvals,vnorms)]
        ...
        ax.set_xticks(x); ax.set_xticklabels(vnames,color='white',fontsize=8,rotation=15)
        ax.tick_params(colors='white',labelsize=8)
        ax.set_title("Patient Vitals vs Normal Range",color='white',fontweight='bold',fontsize=11)
    ...
    with tab3:
        ...
        if risk_diseases:
            ...
            for i,rec in enumerate(["Maintain regular exercise (30 min daily)",
                "Eat a balanced diet rich in fruits and vegetables",
                "Get health checkup every 6 months",
                "Stay hydrated — drink 8 glasses of water daily",
                "Maintain healthy sleep schedule (7-8 hours)"],1):
                st.markdown(f"**{i}.** {rec}")
        ...
        c1,c2=st.columns(2)
        with c1:
            factors={'Smoking':(1 if smoker else 0,1),'Alcohol':(1 if alcohol else 0,1),
                     'Sedentary':(1 if pa_val==0 else 0,1),
                     'Overweight (BMI>25)':(1 if bmi>25 else 0,1),
                     'High Sugar':(1 if blood_sugar>140 else 0,1)}
            fig,ax=plt.subplots(figsize=(5,4),facecolor='#16213e'); ax.set_facecolor('#16213e')
            fnames=list(factors.keys()); fvals=[v[0] for v in factors.values()]
            fcolors=['#e53935' if v==1 else '#2e7d32' for v in fvals]
            ax.barh(fnames,fvals,color=fcolors,edgecolor='#ffffff11',height=0.5)
            ax.set_xlim(0,1.5); ax.set_title("Risk Factors Present",color='white',fontweight='bold',fontsize=10)
            ax.tick_params(colors='white',labelsize=8)
            for sp in ['top','right','bottom']: ax.spines[sp].set_visible(False)
            ax.spines['left'].set_color('#333')
            fig.tight_layout(); st.pyplot(fig,use_container_width=True); plt.close()
        with c2:
            total=sum([1 if smoker else 0,1 if alcohol else 0,
                       1 if pa_val==0 else 0,1 if bmi>25 else 0,1 if blood_sugar>140 else 0])
            st.markdown(f"""
            <div class="info-card" style="margin-top:20px;">
                <h3>{total}/5</h3><p>Lifestyle Risk Factors</p>
            </div>
            <div style="margin-top:15px;color:#aaa;font-size:0.85rem;line-height:1.8;">
                {"🔴 Smoker detected" if smoker else "🟢 Non-smoker"}<br>
                {"🔴 Alcohol consumption" if alcohol else "🟢 No alcohol"}<br>
                {"🔴 Sedentary lifestyle" if pa_val==0 else "🟢 Active lifestyle"}<br>
                {"🔴 BMI above normal" if bmi>25 else "🟢 BMI normal"}<br>
    ...
    with tab4:
        ...
        <div style="background:rgba(0,188,212,0.08);border-radius:12px;
            ...
            <div style="color:#aaa;font-size:0.82rem;line-height:1.6;">
                SHAP shows WHY the AI made each prediction. It reveals which health
                features pushed your risk UP 🔴 or DOWN 🟢 for each disease.
            </div>
        </div>""",unsafe_allow_html=True)

        selected_disease=st.selectbox("Select Disease to Explain",
            ["Diabetes 🩸","Cardiovascular ❤️","Kidney (CKD) 🫘",
             "Hypertension 🩺","Fatty Liver 🫀"],key="shap_disease")
        disease_key_map={"Diabetes 🩸":"diabetes","Cardiovascular ❤️":"cvd",
                         "Kidney (CKD) 🫘":"ckd","Hypertension 🩺":"hypertension",
                         "Fatty Liver 🫀":"fatty_liver"}
        sel_key=disease_key_map[selected_disease]

        shap_btn=st.button("🔍 Generate SHAP Explanation",key="shap_btn")
        ...
        if shap_btn:
            with st.spinner("🔄 Computing SHAP values..."):
                ...
                importance_df=pd.DataFrame({
                    'Feature':feature_cols,'SHAP':sv,
                    'AbsSHAP':np.abs(sv),'Value':patient_arr[0]
            ...
            with c1:
                ...
                colors_shap=['#e53935' if v>0 else '#2e7d32' for v in top_n['SHAP']]
                fig,ax=plt.subplots(figsize=(6,5),facecolor='#16213e'); ax.set_facecolor('#16213e')
                bars=ax.barh(top_n['Feature'][::-1],top_n['SHAP'][::-1],
                             color=colors_shap[::-1],edgecolor='#ffffff11',height=0.6)
                ...
                for sp in ['top','right']: ax.spines[sp].set_visible(False)
                for sp in ['bottom','left']: ax.spines[sp].set_color('#333')
                red_p=mpatches.Patch(color='#e53935',label='Increases Risk')
                grn_p=mpatches.Patch(color='#2e7d32',label='Decreases Risk')
                ax.legend(handles=[red_p,grn_p],fontsize=7.5,facecolor='#1a1a2e',labelcolor='white')
                fig.tight_layout(); st.pyplot(fig,use_container_width=True); plt.close()
            ...
            with c2:
                top6=pd.DataFrame({'Feature':feature_cols,'Importance':fi
                    }).sort_values('Importance',ascending=False).head(6)
                fig,ax=plt.subplots(figsize=(6,5),facecolor='#16213e'); ax.set_facecolor('#16213e')
                donut_colors=['#e53935','#ff7043','#ff9800','#1e88e5','#43a047','#8e24aa']
                wedges,texts,autotexts=ax.pie(top6['Importance'],labels=top6['Feature'],
                    colors=donut_colors,autopct='%1.1f%%',startangle=90,
                ...
                ax.set_title(f"Top 6 Features — {selected_disease}",
                             color='white',fontweight='bold',fontsize=10,pad=10)
                fig.tight_layout(); st.pyplot(fig,use_container_width=True); plt.close()
            ...
            counterfactuals={
                'diabetes':[("Blood Sugar",blood_sugar,140,"mg/dL",blood_sugar>140),
                            ("HbA1c",hba1c,5.7,"%",hba1c>5.7),
                            ("BMI",bmi,25.0,"",bmi>25),
                            ("Physical Activity","Sedentary","Active","",pa_val==0)],
                'cvd':[("Cholesterol",cholesterol,200,"mg/dL",cholesterol>200),
                       ("Blood Pressure",blood_pressure,120,"mmHg",blood_pressure>120),
                       ("Smoking","Yes" if smoker else "No","No","",smoker),
                       ("BMI",bmi,25.0,"",bmi>25)],
                'ckd':[("Creatinine",creatinine,1.2,"mg/dL",creatinine>1.2),
                       ("Blood Pressure",blood_pressure,120,"mmHg",blood_pressure>120),
                       ("Blood Sugar",blood_sugar,140,"mg/dL",blood_sugar>140)],
                'hypertension':[("Blood Pressure",blood_pressure,120,"mmHg",blood_pressure>120),
                                ("BMI",bmi,25.0,"",bmi>25),
                                ("Alcohol","Yes" if alcohol else "No","No","",alcohol),
                                ("Physical Activity","Sedentary","Active","",pa_val==0)],
                'fatty_liver':[("Triglycerides",triglycerides,150,"mg/dL",triglycerides>150),
            ...
            for i,(feat,current,target,unit,is_risk) in enumerate(cfs):
                ...
                if is_risk:
                    ...
                    <div style="background:rgba(229,57,53,0.1);border-radius:12px;
                        ...
                        <div style="color:#ff7043;font-weight:600;font-size:0.88rem;">⚠️ {feat}</div>
                        <div style="color:#aaa;font-size:0.8rem;margin-top:6px;">
                            Current: <span style="color:#ff5252;font-weight:600;">{current}{unit}</span>
                        </div>
                        <div style="color:#aaa;font-size:0.8rem;">
                            Target: <span style="color:#00e676;font-weight:600;">{target}{unit}</span>
                        </div>
                        <div style="color:#ffcc02;font-size:0.78rem;margin-top:6px;">
                            → Changing this could reduce your risk
                        </div>
                    </div>""",unsafe_allow_html=True)
                else:
                    col.markdown(f"""
                    <div style="background:rgba(46,125,50,0.1);border-radius:12px;
                        ...
                        <div style="color:#66bb6a;font-weight:600;font-size:0.88rem;">✅ {feat}</div>
                        <div style="color:#aaa;font-size:0.8rem;margin-top:6px;">
                            Current: <span style="color:#00e676;font-weight:600;">{current}{unit}</span> — Already good!
                        </div>
                    </div>""",unsafe_allow_html=True)
            ...
            st.dataframe(shap_table.reset_index(drop=True),use_container_width=True,height=350)

        else:
            st.info("👆 Select a disease and click **🔍 Generate SHAP Explanation**")
            c1,c2,c3,c4=st.columns(4)
            for col,(icon,title,desc) in zip([c1,c2,c3,c4],[
                ("🌊","Waterfall Plot","Feature contribution chart"),
                ("🌍","Global Importance","Which features matter most"),
                ("💡","Counterfactuals","What to change to reduce risk"),
                ("📋","SHAP Table","Full feature impact breakdown"),
            ]):
                col.markdown(f"""
                <div style="background:rgba(255,255,255,0.05);border-radius:12px;
                     padding:18px;text-align:center;border:1px solid rgba(255,255,255,0.1);">
    ...
    with tab5:
        ...
        <div style="background:rgba(255,152,0,0.08);border-radius:12px;
            ...
            <div style="color:#aaa;font-size:0.82rem;">
                Click any language button to hear your complete health report.
                Works best in Chrome browser.
            </div>
        </div>""",unsafe_allow_html=True)

        en_text=get_voice_report(patient_name,age,gender,"",0,
                                  probabilities,predictions,all_recs,'en')
        ta_text=get_voice_report(patient_name,age,gender,"",0,
                                  probabilities,predictions,all_recs,'ta')
        hi_text=get_voice_report(patient_name,age,gender,"",0,
                                  probabilities,predictions,all_recs,'hi')
        st.components.v1.html(get_language_html(en_text,ta_text,hi_text),height=280)

        st.markdown("---")
        st.markdown('<div class="section-header">📝 Voice Script Preview</div>',
                    unsafe_allow_html=True)
        lang_preview=st.radio("Preview Language",["English","Tamil","Hindi"],horizontal=True)
        if lang_preview=="English":   st.text_area("English Script",en_text,height=200)
        elif lang_preview=="Tamil":   st.text_area("Tamil Script",ta_text,height=200)
        else:                         st.text_area("Hindi Script",hi_text,height=200)
    ...
    with tab6:
        ...
        <div class="llm-card">
            ...
            <div style="color:#aaa;font-size:0.82rem;line-height:1.6;">
                Get a personalized health report generated by advanced AI in your
                preferred language. Explains risks in simple words with actionable advice.
            </div>
        </div>""",unsafe_allow_html=True)

        c1,c2=st.columns([2,1])
        with c1:
            llm_language=st.selectbox("Select Report Language",
                ["English","Tamil (தமிழ்)","Hindi (हिंदी)"],key="llm_lang")
        with c2:
        ...
        if generate_btn:
            ...
            with st.spinner("🤖 Claude AI is generating your personalized report..."):
                try:
                    ...
                    <div style="background:rgba(255,255,255,0.05);border-radius:15px;
                         padding:25px;border:1px solid rgba(255,255,255,0.1);margin:15px 0;">
                    """,unsafe_allow_html=True)
                    ...
                    <div style="text-align:center;padding:15px;background:rgba(255,152,0,0.08);
                         border-radius:12px;border:1px solid rgba(255,152,0,0.2);">
                        <div style="color:#ffcc02;font-weight:600;margin-bottom:10px;">🎙️ Listen to AI Report</div>
                        <button onclick="speakAI()" style="background:linear-gradient(135deg,#ff9800,#ff5722);
                            color:white;border:none;border-radius:25px;padding:10px 22px;
                            font-size:0.88rem;font-weight:600;cursor:pointer;margin:4px;">🔊 Speak</button>
                        <button onclick="window.speechSynthesis.cancel()" style="background:linear-gradient(135deg,#424242,#212121);
                            color:white;border:none;border-radius:25px;padding:10px 22px;
                            font-size:0.88rem;font-weight:600;cursor:pointer;margin:4px;">⏹️ Stop</button>
                        <button onclick="window.speechSynthesis.pause()" style="background:linear-gradient(135deg,#1565c0,#0d47a1);
                            color:white;border:none;border-radius:25px;padding:10px 22px;
                            font-size:0.88rem;font-weight:600;cursor:pointer;margin:4px;">⏸️ Pause</button>
                        <button onclick="window.speechSynthesis.resume()" style="background:linear-gradient(135deg,#1b5e20,#2e7d32);
                            color:white;border:none;border-radius:25px;padding:10px 22px;
                            font-size:0.88rem;font-weight:600;cursor:pointer;margin:4px;">▶️ Resume</button>
                    ...
                    function speakAI(){{
                        ...
                        utt.lang='{lang_map.get(lang_code,"en-US")}'; utt.rate=0.9;
                    ...
                    st.download_button("⬇️ Download AI Report",data=llm_report_text,
                        file_name=f"{patient_name}_AI_report_{lang_code}.txt",
                        mime="text/plain",key="llm_dl")
            ...
            st.info("👆 Select language and click **🤖 Generate AI Report**")
        ...
        if st.button("✨ Generate Quick Summary",key="sum_btn"):
            with st.spinner("Generating summary..."):
                try:
                    summary=generate_llm_summary(predictions,probabilities,patient_name)
                    st.markdown(f"""
    ...
    with tab7:
        ...
        risk_names_str=", ".join([disease_info[d][1] for d in risk_diseases]) if risk_diseases else "None"
        report=f"""
FedHealth-Twin: Multi-Disease Health Report
Generated : {now}  |  Patient: {patient_name}
{'='*60}
PATIENT DETAILS:
  Age: {age} yrs  |  Gender: {gender}  |  BMI: {bmi}
  Blood Pressure: {blood_pressure} mmHg  |  Blood Sugar: {blood_sugar} mg/dL
  HbA1c: {hba1c}%  |  Cholesterol: {cholesterol} mg/dL
  Triglycerides: {triglycerides} mg/dL  |  ALT Enzyme: {alt_enzyme} U/L
  Heart Rate: {heart_rate} bpm  |  SpO2: {spo2}%  |  Creatinine: {creatinine} mg/dL
  Smoker: {'Yes' if smoker else 'No'}  |  Diabetes Hx: {'Yes' if diabetes_history else 'No'}
  Family Hx: {'Yes' if family_history else 'No'}  |  Alcohol: {'Yes' if alcohol else 'No'}
{'='*60}
DISEASE PREDICTIONS:
...
"""
        ...
        with c1:
            st.download_button("⬇️ Download Full Report",data=report,
                ...
                mime="text/plain",key="dl1")
        with c2:
            en_script=get_voice_report(patient_name,age,gender,"",0,
                                        probabilities,predictions,all_recs,'en')
            st.download_button("⬇️ Download Voice Script",data=en_script,
                file_name=f"{patient_name}_voice_script.txt",
                mime="text/plain",key="dl2")
...
# ─── Welcome Screen ──────────────────────────────────────────
    ...
    for col,(icon,title,desc) in zip([c1,c2,c3,c4,c5],[
        ...
        ("🔍","SHAP & XAI","Explainable AI per disease"),
        ("🎙️","Voice Tab","EN | Tamil | Hindi"),
        ("🤖","AI Report Tab","Claude AI report"),
        ("📄","Download Tab","Full report download"),
    ]):
        col.markdown(f"""
        <div style="background:rgba(255,255,255,0.05);border-radius:15px;
             padding:18px;text-align:center;border:1px solid rgba(255,255,255,0.1);">
    ...
    <div style="text-align:center;color:#444;padding:16px;font-size:0.85rem;">
        ❤️ FedHealth-Twin | Multi-Disease AI | Multilingual Voice | Claude AI Reports
    </div>""",unsafe_allow_html=True)